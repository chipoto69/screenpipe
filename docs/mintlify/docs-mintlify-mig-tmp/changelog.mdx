...
- **cross-device memories sync** (Pro, opt-in) — your AI memories now sync across the machines you sign in on, with last-write-wins merge and per-device provenance so you can see where each memory came from.
- **markdown-first meeting notes** — the meeting note editor now renders Obsidian-style live markdown: type `#`, `-`, `**bold**`, links, and they format inline as you write. see [meeting transcription](/meeting-transcription).
- **copy meeting + transcript** — one-click button on a meeting copies the note and full transcript to your clipboard, ready to paste into email, Slack, or your tool of choice.
- **smarter meeting summaries** — summarize now refreshes the meeting title and asks before overwriting an existing summary, so it won't clobber notes you've already written.
- **connections show up in chat & pipes context** — Pi (the chat assistant) and your pipes now know which integrations you've connected, so they can answer questions like "do I have Slack connected?" without guessing. see [connections](/connections).
- **Vercel via OAuth** — the Vercel connection has migrated from API tokens to OAuth2.0; click connect in **settings → [connections](/connections)** to authorize.
- **PostHog: EU + self-hosted support** — the PostHog connection now has a host field so you can point it at `eu.posthog.com` or your self-hosted instance. blank defaults to `us.posthog.com`. see [connections](/connections).
- **PII redaction for images** — the [privacy filter](/privacy-filter) now redacts personal info from screenshots and OCR text in the background, on top of the existing chat-message redaction.
- **enclave attestation for the privacy filter** — the privacy filter now verifies the Tinfoil enclave's attestation before sending anything to it, so you have cryptographic proof your data is going where it's supposed to.
- **gpt-5.5 is the default ChatGPT preset** — new installs (and unmodified presets) now point at gpt-5.5 instead of gpt-5.4.

### updates

- **better battery on macOS** — screenpipe now skips its accessibility scan while the screen is locked, cutting wake-ups when you're away from the machine.

### bug fixes

- **chat sidebar ordering** — chats you've actually opened or sent messages in now sort above background pipe completions, so your active conversations stay on top.
- **Gemini tool calls** — fixed a tool-schema recursion bug and the gateway now cascades past 404/403 to a working model instead of failing the request.
- **Anthropic tool calls** — `tool_use.id` is now sanitized to match Anthropic's required pattern, fixing sporadic 400s on Claude models.
- **Calendar permission "Fix" button** — on macOS, the Fix button now actually re-prompts for Calendar access (it was a silent no-op for some users after app updates).
- **QuickBooks Dev tab** — Dev-tab Intuit tokens now correctly hit the sandbox host instead of production.
- **onboarding errors are readable** — auth errors during onboarding now show the real message instead of `{}`, and a silent API-key rotation is logged loudly.
- **overlay polish** — the app icon and phone icon in the overlay pill no longer expand it on hover.
- **search suggestions** — fixed a regression where suggestions stopped appearing because they referenced a dropped accessibility table.
